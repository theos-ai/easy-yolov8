...
from ultralytics.utils import (DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, RANK, ROOT, SETTINGS, callbacks,
...
class Model:
    ...
    def __init__(self, model: Union[str, Path] = 'yolov8n.pt', task=None, dataset_path='./datasets', experiment_path='./experiments', device=None) -> None:
        """
        ...
        if not suffix and Path(model).stem in GITHUB_ASSET_STEMS:
            model, suffix = Path(model).with_suffix('.weights'), '.weights'  # add suffix, i.e. yolov8n -> yolov8n.pt
        if suffix in ('.yaml', '.yml'):
            self._new(model, task)
        else:
            self._load(model, task, device=device)
    ...
    def _load(self, weights: str, task=None, device=None):
        """
        Initializes a new model and infers the task type from the model head.
        ...
        if suffix == '.pt' or suffix == '.weights':
    ...
    def train(self, trainer=None, logger=None, **kwargs):
        """
        ...
        self.trainer = trainer(overrides=overrides, _callbacks=self.callbacks, logger=logger)
